...
# ack control
def ack_control(file_bytes_array, start:int, end:int):
    pass
...
def byte_addition(file_bytes_array, start:int, end:int):
    ...
    for i in range(start, end):
        if(i) == len(file_bytes_array):
            # reached end of array
            break
        byte_stream += file_bytes_array[i]
    
    # byte_stream = f'{start}-{end} '.encode('utf-8')+ byte_stream
    return byte_stream



# the congestion control itself
