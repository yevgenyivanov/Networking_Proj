...
def rudp_image_decoder(new_client_socket: socket.socket, total_elems_response: int, filename: str):
    file_bytes_array = [None] * total_elems_response
    print("reached rudp_image_decoder")
    for i in range(int(total_elems_response)):

        # handle disconnects/timeouts/decode errors
        try:
            new_client_socket.settimeout(3)
            bytes_response = new_client_socket.recv(1024*(pow(2, i)))
            # first timeout: try allowing resend current packet
        except TimeoutError:
            print("socket timed out, from no response")
            # send NO_ACK message to fileserver if no response was met
            bytes_response = new_client_socket.send(
                f"NO_ACK_{i}".encode('utf-8'))
            new_client_socket.settimeout(3)
            try:
                bytes_response = new_client_socket.recv(1024*(pow(2, i)))
                if bytes_response != None:
                    bytes_response = new_client_socket.send(
                        "ACK".encode('utf-8'))
                if bytes_response == None and file_bytes_array[i-1] != None:
                    # reached end of file
                    break
            except TimeoutError:
                print("no response for too long, returning")
                return
            except:
                print("error encoding or decoding with image file")
            break

        if 'END_OF_FILE' in bytes_response.decode('utf-8'):
            print("received all file exiting")
            break
        file_bytes_array[i] = bytes_response
        print("appended bytes")

    # appending file_data from array to
    counter = 0
    byte_stream = b''
    ...
    if (protocol == 'RUDP' or protocol == 'rudp'):
        ...
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as new_client_socket:
                ...
                total_elems_response = new_client_socket.recv(
                    1024).decode('utf-8')
                print("Total amount of elements in file_bytes_array: ",
                      total_elems_response)
                ...
                for i in range(int(total_elems_response)):
                    ...
                    try:
                        new_client_socket.settimeout(3)
                        bytes_response = new_client_socket.recv(
                            1024*(pow(2, i)))
                        # first timeout: try allowing resend current packet
                    except TimeoutError:
                        print("socket timed out, from no response")
                    ...
                    file_bytes_array.append(bytes_response)
                # appending file_data from array to
                counter = 0
                byte_stream = b''
                while (counter < int(total_elems_response)):
                    if file_bytes_array[counter] == None:
                        counter+=1
                        continue
                    byte_stream += file_bytes_array[counter]
                    counter += 1
